...
from bin import SATELLiTES_gui
from gui.tabs_2reagents import *
from gui.tabs_3reagents import *
...
if len(sys.argv[0].split("/"))>1:
    path_SATELLiTES = "/".join(sys.argv[0].split("/")[:-1]) + "/"
    path_SATELLiTES_img = path_SATELLiTES + "img/"
else:
    path_SATELLiTES = "\\".join(sys.argv[0].split("\\")[:-1]) + "\\"
    path_SATELLiTES_img = path_SATELLiTES + "img\\"
...
def launch_function(reagentsA, reagentsB, reagentsC, reaction, output_path, nb_cpu, modeA, modeB, modeC, inputA, inputB, inputC, step, step_path):
    ...
    if modeC == "":
        SATELLiTES_gui.CB_run_2reagents(reagentsA, reagentsB, reaction, output_path, nb_cpu, modeA, modeB, inputA, inputB, window, app, job_name = "SATELLiTES")
    else:
        SATELLiTES_gui.CB_run_3reagents(reagentsA, reagentsB, reagentsC, reaction, output_path, nb_cpu, modeA, modeB, modeC, inputA, inputB, inputC, step, step_path, window, app, job_name = "SATELLiTES")
...
class Window(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        app_icon = QIcon()
        app_icon.addFile(path_SATELLiTES_img + 'SATELLiTES_favicon.png', QSize(256,256))
        self.setWindowIcon(app_icon)
        self.setWindowTitle("SATELLiTES - Alpha 2")
        ...
        path_logo_SATELLiTES = path_SATELLiTES_img + "SATELLiTES_header.png"
        self.pixmap_logo = QPixmap(path_logo_SATELLiTES)
...
if __name__ == "__main__":
    ...
    qqq = QIcon('img/SATELLiTES_favicon.png')
