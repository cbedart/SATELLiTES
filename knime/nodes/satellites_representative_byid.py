...
@knext.node(name="Representative - By ID", node_type=knext.NodeType.SOURCE, icon_path="icon.png", category=category_utilities)
...
class Representative_byID:
    """ **SATELLiTES - Representative by ID**
    ...
    For this specific node, the representative compound will be selected from an **user-specified compound ID** available in the input set.

    The next step will include the \"X-reagents - Step #X\" set of nodes.
    """

    byid_param = knext.StringParameter("Representative ID", "ID of the reagent that will be used as the representative.")
    ...
    def execute(self, execute_context, input_1):
        ...
        output_temp.loc[output_temp["ID"] == self.byid_param, "Representative"] = True
