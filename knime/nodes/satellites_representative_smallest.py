...
@knext.node(name="Representative - Smallest", node_type=knext.NodeType.SOURCE, icon_path="icon.png", category=category_utilities)
...
class Representative_smallest:
    """ **SATELLiTES - Smallest MW representative**
    ...
    For this specific node, the representative compound will be **automatically selected** from the input set based on **molecular weights** : the smallest compound will be chosen as the representative compound.
    ...
    def execute(self, execute_context, input_1):
        ...
        os.makedirs(workdir_temp + "/smallest_results/", exist_ok=True)
        stgui.CB_smallest_finder(input_1_temp, workdir_temp + "/smallest", "A", 0)
        smallest_temp = pd.read_csv(workdir_temp + "/smallest_results/smallest_input_smallest.smi", sep = "\t", header = None)
        ...
        output_temp.loc[output_temp["ID"] == smallest_temp.iloc[0,1],"Representative"] = True
